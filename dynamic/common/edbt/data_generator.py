...
class EdbtDataGenerator(DataGenerator):
    """
    Data generator for the EDBT schema.
    ...
    def _generate_data(self):
        ...
        self.__generate_sellers(c.seller)
        self.__generate_categories(c.category)

        # Products
        products = self.__generate_products(c.product, c.seller)

        # Many-to-many / graphy tables
        self.__generate_has_category(c.product, c.category)
        self.__generate_has_interest(c.person, c.category)
        self.__generate_follows(c.person, c.follows)

        # Skewed sampler for product picks in orders
        weights = self.__build_product_weights(c.product)
        product_sampler = self._create_sampler(weights)

        # Orders and order items (stream)
        customer_products = self.__generate_customers_orders_items(
            c.order,
            persons,
            products,
            product_sampler,
    ...
    def __generate_sellers(self, n_sellers: int) -> None:
    ...
    def __generate_categories(self, n_categories: int) -> None:
        """
    ...
    def __build_product_weights(self, n_products: int) -> list[float]:
        """
        Heavy skew:
        - top 1% are hot
    ...
    def __generate_products(self, n_products: int, n_sellers: int) -> list[list]:
        """
        Returns:
    ...
    def __generate_has_category(self, n_products: int, n_categories: int) -> None:
    ...
    def __generate_has_interest(self, n_persons: int, n_categories: int) -> None:
    ...
    def __generate_follows(self, n_persons: int, n_edges: int) -> None:
        """
    ...
    def __generate_reviews(self, n_reviews: int, customer_products: list[tuple[datetime, list[int]]]) -> None:
        """
        Reviews are big.
