...
class MongoTpchLoader(MongoLoader):
    ...
    def _get_constraints(self):
        return [
            ...
            MongoIndex('customer', [ 'c_mktsegment', 'c_acctbal' ]),
            MongoIndex('customer', [ 'c_acctbal' ]),
            MongoIndex('customer', [ 'nation.region.r_name', 'c_acctbal' ]),
            MongoIndex('orders', [ 'o_orderkey' ], is_unique=True),
            MongoIndex('orders', [ 'o_custkey' ]),
            MongoIndex('orders', [ 'o_custkey', 'o_orderdate' ]),
            MongoIndex('orders', [ 'o_orderdate' ]),
            MongoIndex('orders', [ 'o_orderstatus', 'o_orderpriority', 'o_orderdate' ]),
            MongoIndex('orders', [ 'o_totalprice', 'o_orderdate' ]),
            MongoIndex('orders', [ 'lineitems.l_shipdate' ]),
            MongoIndex('orders', [ 'lineitems.l_partkey', 'o_orderdate' ]),
            MongoIndex('orders', [ 'lineitems.l_suppkey', 'o_orderdate' ]),
            MongoIndex('part', [ 'p_partkey' ], is_unique=True),
            MongoIndex('part', [ 'p_brand', 'p_size', 'p_retailprice' ]),
            MongoIndex('part', [ 'p_size' ]),
            MongoIndex('part', [ 'partsupps.ps_suppkey' ]),
            MongoIndex('part', [ 'partsupps.ps_supplycost' ]),
            MongoIndex('part', [ 'partsupps.ps_availqty' ]),
            MongoIndex('supplier', [ 's_suppkey' ], is_unique=True),
            MongoIndex('supplier', [ 's_nationkey' ]),
            MongoIndex('supplier', [ 's_acctbal' ]),
            MongoIndex('supplier', [ 'nation.n_nationkey' ]),
            MongoIndex('supplier', [ 'nation.region.r_regionkey' ]),
            MongoIndex('supplier', [ 'nation.region.r_name', 's_acctbal' ]),
            MongoIndex('partsupp', [ 'ps_partkey', 'ps_suppkey' ], is_unique=True),
            MongoIndex('partsupp', [ 'ps_partkey' ]),
            MongoIndex('partsupp', [ 'ps_suppkey' ]),
            MongoIndex('partsupp', [ 'ps_suppkey', 'ps_supplycost' ]),
            MongoIndex('partsupp', [ 'ps_supplycost' ]),
            MongoIndex('partsupp', [ 'ps_availqty' ]),
            ...
            MongoIndex('lineitem', [ 'l_shipmode', 'l_shipdate' ]),
            MongoIndex('lineitem', [ 'l_quantity' ]),
        ]
