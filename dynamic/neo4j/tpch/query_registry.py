...
def export():
    return Neo4jTpchQueryRegistry()

class Neo4jTpchQueryRegistry(TpchQueryRegistry[str]):
    ...
    @query('join-0', 'Pricing Summary Report')
    ...
    @query('join-1', 'Local Supplier Volume')
    ...
    @query('join-2', 'Forecasting Revenue Change')
    ...
    @query('join-3', 'Returned Item Reporting (Fix: Date generation)')
    ...
    @query('join-4', 'Shipping Modes and Order Priority (Fix: Shipmode generation)')
    ...
    @query('join-5', 'Promotion Effect (Fix: Month range and end date logic)')
    ...
    @query('join-6', 'Discounted Revenue (Fix: Brand generation)')
    ...
    @query('join-8', 'Parts and Suppliers')
    ...
    @query('join-11', 'Customer Nation Analysis')
    ...
    @query('join-12', 'Part Lineitem Summary')
    ...
    @query('scan-0', 'Simple Scan (All Nodes of a Label)')
    def _simple_scan(self):
        # TODO param
        return f'MATCH (n:Nation) RETURN n.n_name, n.n_comment LIMIT 100'

    @query('scan-1', 'Scan with Exact Property Match')
    ...
    @query('scan-2', 'Scan with Numeric Filter')
    ...
    @query('scan-7', 'Scan with Date Filter')
    ...
    @query('n-hop-0', '1-Hop Traversal (Find orders for a customer)')
    ...
    @query('n-hop-1', '1-Hop with Filter and Aggregation (Count items in high-priority orders)')
    ...
    @query('n-hop-2', '2-Hop Traversal (Find items for a customer)')
    ...
    @query('n-hop-3', '2-Hop with Aggregation (Count orders per nation)')
    ...
    @query('n-hop-4', '3-Hop Traversal (Find parts from a supplier)')
    ...
    @query('n-hop-5', '3-Hop with Property Filter (Customers in a region)')
    ...
    @query('n-hop-6', 'Complex Path (4-Hop) and Aggregation')
    ...
    @query('n-hop-7', 'Multi-hop with CONTAINS (Find orders for a part type)')
    ...
    @query('n-hop-8', 'Aggregation on Traversal (Supplier stock value)')
    ...
    @query('n-hop-9', 'Complex Path with Multiple Filters')
    ...
    @query('optional-0', 'Suppliers with optional revenue from delivered items')
    ...
    @query('optional-1', 'Nations with optional export activity')
    ...
    @query('optional-2', 'Customers with optional recent orders')
    ...
    @query('optional-3', 'Customers with optional supplier diversity')
    def _optional1(self):
        return '''
            MATCH (c:Customer)-[:PLACED]->(o:Orders)
    ...
    @query('optional-4', 'Customers with optional regional purchasing patterns')
    def _optional2(self):
        return '''
            MATCH (c:Customer)
    ...
    @query('optional-5', 'Orders with optional supplier competition')
    def _optional3(self):
        return '''
            MATCH (o:Orders)
            OPTIONAL MATCH (o)-[:HAS_ITEM]->(l:LineItem)-[:SUPPLIED_BY]->(s:Supplier)
    ...
    @query('optional-6', 'Customers with optional unfulfilled supplier relationships')
    def _optional4(self):
        return '''
    ...
    @query('basic-0', 'Q1 variants (Original)')
    def _basic1(self):
        return '''
            MATCH (li:LineItem)
    ...
    @query('basic-1', 'Q5 variant (Original)')
    def _basic2(self):
        return '''
            MATCH (c:Customer)-[:PLACED]->(o:Orders)-[:HAS_ITEM]->(li:LineItem),
    ...
    @query('basic-2', 'Q6 variant (Original)')
    def _basic3(self):
        return '''
            MATCH (li:LineItem)
    ...
    @query('basic-3', 'Q10 variant (Original)')
    def _basic4(self):
        return '''
            MATCH (c:Customer)-[:PLACED]->(o:Orders)-[:HAS_ITEM]->(li:LineItem),
    ...
    @query('basic-4', 'Simple aggregation query (Original)')
    def _basic5(self):
        return '''
            MATCH (li:LineItem)
    ...
    @query('basic-5', 'Simple scan with limit (Original)')
    def _basic6(self):
        return '''
            MATCH (c:Customer)
    ...
    @query('complex-join-0', 'Customer Segment Revenue')
    ...
    @query('complex-join-1', 'Supplier Revenue Analysis')
    ...
    @query('complex-join-2', 'Part Supplier Customer Chain')
    ...
    @query('complex-join-3', 'Multi-way with Partsupp')
    ...
    @query('complex-join-4', 'Full Chain Analysis')
    ...
    @query('complex-join-5', 'Regional Supply Chain')
    ...
    @query('selective-0', 'Discount Range')
    ...
    @query('selective-1', 'High Value Orders')
    ...
    @query('selective-2', 'Premium Customers')
    ...
    @query('selective-3', 'Specific Part Types')
    ...
    @query('selective-4', 'Late Shipments')
    ...
    @query('selective-5', 'Low Supply Cost')
    ...
    @query('large-0', 'Lineitems by Price')
    ...
    @query('large-3', 'Customer Balance Ranking')
    ...
    @query('large-4', 'Lineitem Quantity Sort')
    ...
    @query('large-5', 'Recent Shipments')
    ...
    @query('having-0', 'Large Order Aggregates')
    ...
    @query('having-1', 'High Volume Customers')
    ...
    @query('having-2', 'Popular Parts by Brand')
    ...
    @query('having-3', 'High Revenue Suppliers')
    ...
    @query('having-4', 'Part Categories with High Sales')
    ...
    @query('having-5', 'Customer Segments with Volume')
    ...
    @query('argument-0', 'Customers ordering parts they never reordered')
    ...
    @query('argument-1', 'Parts ordered from suppliers outside their region')
    ...
    @query('argument-2', 'Customers with orders but no line items')
    ...
    @query('argument-3', 'Suppliers that never served certain customers')
    ...
    @query('triadic-1', 'High-value customer network recommendations')
    ...
    @query('order-agg-0', 'Revenue per nation, ordered by nation name')
    ...
    @query('order-agg-1', 'Orders per customer, ordered by customer name')
    ...
    @query('index-scan-0', 'Filter by non-null date (should use index)')
    ...
    @query('index-scan-1', 'Filter by modulo on an indexed numeric property (should use index)')
    ...
    @query('index-scan-2', 'LineItems shipped on Mondays')
    ...
    @query('index-scan-3', 'PartSupp buckets (there is no deeper meaning)')
    ...
    @query('order-0', 'Customers by nation')
    ...
    @query('order-1', 'Parts by usage count')
