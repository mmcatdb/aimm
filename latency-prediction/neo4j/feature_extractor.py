...
class FeatureExtractor:
    ...
    def _parse_details(self, details: str) -> dict[str, Any]:
        """
    ...
    def build_vocabularies(self, plans: list[dict]):
        ...
        def traverse(node):
            ...
            if details:
                ...
                details_info = self._parse_details(details)
    ...
    def encode_multi_hot(self, values: list[str], vocabulary: set) -> np.ndarray:
        """
    ...
    def extract_features(self, node: dict) -> np.ndarray:
        ...
        if estimated_rows is None:
            estimated_rows = 0
        estimated_rows_norm = self.normalize_numeric(float(estimated_rows), 'EstimatedRows')
        ...
        if self.identifiers_vocab:
            identifiers_vec = self.encode_multi_hot(identifiers, self.identifiers_vocab)
            features.append(identifiers_vec)

        # Number of identifiers (normalized)
        num_identifiers = len(identifiers)
        num_identifiers_norm = self.normalize_numeric(float(num_identifiers), 'num_identifiers')
        features.append(np.array([num_identifiers_norm], dtype=np.float32))

        # Number of children (normalized)
        children = node.get('children', [])
        num_children = len(children)
        num_children_norm = self.normalize_numeric(float(num_children), 'num_children')
        ...
        if details:
            details_length_norm = self.normalize_numeric(float(len(details)), 'details_length')
        else:
        ...
        num_vars_norm = self.normalize_numeric(float(details_info['num_variables']), 'num_variables')
