...
class FeatureExtractor:
    ...
    def build_vocabularies(self, plans: List[Dict]):
        ...
        def traverse_plan(node):
            ...
            for key in ['Plan Width', 'Plan Rows', 
                       'Plan Buffers', 'Estimated I/Os', 'Hash Buckets',
                        'Sort Space Used', 'Total Cost']:
                if key in node:
    ...
    def extract_common_features(self, node: Dict) -> np.ndarray:
        ...
        features.append(self.normalize_numeric(
            node.get('Plan Width', 0), 'Plan Width'))
        features.append(self.normalize_numeric(
            node.get('Plan Rows', 0), 'Plan Rows'))
        features.append(self.normalize_numeric(
            node.get('Total Cost', 0), 'Total Cost'))
        features.append(self.normalize_numeric(
            node.get('Plan Buffers', 0), 'Plan Buffers'))
        features.append(self.normalize_numeric(
            node.get('Estimated I/Os', 0), 'Estimated I/Os'))
        features.append(self.normalize_numeric(
            node.get('Startup Cost', 0), 'Startup Cost'))
