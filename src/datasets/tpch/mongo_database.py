...
class TpchMongoDatabase(TpchDatabase[MongoQuery]):
    ...
    @query('basic', 1.0, 'lineitem date range with various operators and limits')
    ...
    @query('basic', 0.5, 'lineitem quantity filter (more selectivity diversity)')
    ...
    @query('basic', 1.0, 'orders price range')
    ...
    @query('basic', 1.0, 'orders price threshold sorted')
    ...
    @query('index-scan', 1.0, 'orders point lookup')
    def _orders_point_lookup(self):
        return MongoFindQuery('orders',
            filter={'o_orderkey': self._param_orderkey()},
        )

    @query('index-scan', 1.0, 'orders multi-key lookup')
    ...
    @query('sort', 1.0, 'orders sorted by price')
    ...
    @query('sort', 1.0, 'lineitem sorted by price')
    ...
    @query('group', 1.0, 'lineitem group by returnflag')
    ...
    @query('group', 1.0, 'orders group by priority')
    ...
    @query('customer', 1.0, 'customer segment balance')
    ...
    @query('customer', 0.5, 'customer balance only (varying selectivity)')
    ...
    @query('part', 0.5, 'part brand size filter')
    ...
    @query('supplier', 1.0, 'supplier balance filter')
    ...
    @query('supplier', 0.5, 'supplier balance range')
    ...
    @query('supplier', 0.25, 'supplier nation filter')
    ...
    @query('full-scan', 0.25, 'nation full scan')
    def _nation_full(self):
        return MongoFindQuery('nation')

    @query('full-scan', 0.25, 'region full scan')
    def _region_full(self):
        return MongoFindQuery('region')

    @query('full-scan', 0.25, 'Customer full scan (30K rows)')
    def _customer_full(self):
        return MongoFindQuery('customer')

    @query('full-scan', 0.25, 'Partsupp full scan (160K rows)')
    def _partsup_full(self):
        return MongoFindQuery('partsupp')

    @query('full-scan', 0.25, 'Supplier full scan (2K rows)')
    ...
    @query('skip', 0.5, 'lineitem + skip + limit')
    ...
    @query('projection', 0.5, 'customer projection lookup')
    ...
    @query('agg-complex', 1.0, 'TPC-H Q1 variants with varying date ranges (from narrow to full-table scan)')
    ...
    @query('agg-complex', 1.0, 'TPC-H Q1 with $lte (matching evaluation style)')
    ...
    @query('agg-complex', 0.5, 'Simple lineitem aggregation (count + sum only, less accumulators)')
    ...
    @query('agg-complex', 0.5, 'Add partsupp queries (multiple patterns including just cost filter)')
    ...
    @query('agg-complex', 1.0, 'Partsupp with just cost filter + sort (matches evaluation)')
    ...
    @query('agg-complex', 0.5, 'Supplier without sort (matches evaluation pattern)')
    ...
    #endregion
    #region Baseline
    # (kept from original for coverage)

    @query('basic', 1.0, 'Lineitem shipdate filter')
    ...
    @query('basic', 1.0, 'Orders price filter + sort')
    ...
    @query('basic', 1.0, 'Customer segment balance filter')
    ...
    @query('basic', 1.0, 'Part brand size filter')
    ...
    @query('basic', 1.0, 'Supplier balance filter')
    def _supplier_balance(self):
        return MongoFindQuery('supplier',
            filter={'s_acctbal': {'$gt': self._param_int('balance', 2000, 8000)}},
        )

    @query('basic', 1.0, 'Partsupp cost filter + sort')
    ...
    @query('group', 1.0, 'TPC-H Q1 date filter')
    def _tpch_date(self):
        return MongoAggregateQuery('lineitem', [
    ...
    @query('group', 1.0, 'Orders group priority year filter')
    ...
    @query('predicate', 1.0, 'Orders $or extreme prices')
    ...
    @query('predicate', 1.0, 'Customer $or segment + balance')
    ...
    @query('predicate', 1.0, 'Lineitem $or returnflag|shipmode + quantity')
    ...
    @query('predicate', 1.0, 'Lineitem $and 4-pred (TPC-H Q6 style)')
    ...
    @query('predicate', 1.0, 'Orders $and date + priority + price')
    ...
    @query('predicate', 1.0, 'Orders status $ne F')
    ...
    @query('predicate', 1.0, 'Lineitem $ne returnflag + shipmode')
    ...
    @query('predicate', 1.0, 'Lineitem shipmode $nin 3 values')
    ...
    @query('predicate', 1.0, 'Orders priority $nin urgent/high')
    ...
    @query('predicate', 1.0, 'Customer $exists comment + high balance')
    ...
    @query('predicate', 1.0, 'Customer $regex name prefix')
    ...
    @query('predicate', 1.0, 'Part $regex type ends BRASS')
    def _part_regex_type(self):
        return MongoFindQuery('part',
            filter={'p_type': {'$regex': 'BRASS$'}},
        )

    @query('predicate', 1.0, 'Part $regex name contains green')
    ...
    @query('predicate', 1.0, 'Lineitem nested $and/$or (TPC-H Q19 style)')
    ...
    @query('selective', 1.0, 'Orders extreme high price (near empty)')
    def _orders_extreme_prices(self):
        return MongoFindQuery('orders',
            filter={'o_totalprice': {'$gt': 490000}},
        )

    @query('selective', 1.0, 'Customer balance > 9990 (near empty)')
    def _customer_balance_nin(self):
        return MongoFindQuery('customer',
            filter={'c_acctbal': {'$gt': 9990}},
        )

    @query('selective', 1.0, 'Lineitem exact quantity = 1 disc = 0 tax = 0 (near empty)')
    ...
    @query('permissive', 1.0, 'Lineitem quantity > 0 (full table, 1.2M rows)')
    def _lineitem_quantity_positive(self):
        return MongoFindQuery('lineitem',
            filter={'l_quantity': {'$gt': 0}},
        )

    @query('permissive', 1.0, 'Orders price > 0 (full table, 300K rows)')
    ...
    @query('unlimited', 1.0, 'Customer balance > 0 sorted NO LIMIT')
    ...
    @query('unlimited', 1.0, 'Orders 1997+ sorted by price NO LIMIT')
    ...
    @query('unseen', 1.0, 'Lineitem commitdate < 1993-06 LIMIT 200')
    ...
    @query('unseen', 1.0, 'Lineitem receiptdate >= 1998-06')
    ...
    @query('unseen', 1.0, 'Lineitem 3-date cross filter')
    ...
    @query('unseen', 1.0, 'Orders status filter')
    ...
    @query('unseen', 1.0, 'Orders exact clerk lookup')
    def _orders_exact_clerk_lookup(self):
        return MongoFindQuery('orders',
            filter={'o_clerk': 'Clerk#000000951'},
        )

    @query('unseen', 1.0, 'Lineitem shipinstruct + mode (string equality combo)')
    ...
    @query('skip', 1.0, 'Orders deep pagination skip = 5000')
    ...
    @query('skip', 1.0, 'Lineitem deep skip = 10000')
    ...
    @query('skip', 1.0, 'Customer skip = 15000 sorted')
    ...
    @query('in-indexed', 1.0, 'Orders 50-key $in lookup')
    def _orders_50_key_in_lookup(self):
        return MongoFindQuery('orders',
            filter={'o_orderkey': {'$in': self._param_orderkeys(50)}},
        )

    @query('in-indexed', 1.0, 'Orders 200-key $in lookup')
    ...
    @query('in-other', 1.0, 'Lineitem shipmode $in [AIR,RAIL]')
    ...
    @query('in-other', 1.0, 'Part 9-brand $in lookup')
    ...
    # G1. $count stage
    @query('count', 1.0, 'Lineitem $count AIR 1997+')
    ...
    @query('count', 1.0, 'Orders $count status = P')
    ...
    @query('group', 1.0, 'Lineitem group shipmode min/max/first/last')
    ...
    @query('group', 1.0, 'Orders group by custkey top-20 spenders (o_custkey -> many groups)')
    ...
    @query('group', 1.0, 'Lineitem $project revenue then group (4 stages)')
    ...
    @query('group', 1.0, 'Customer group by segment avg/max balance')
    ...
    @query('group', 1.0, 'Orders double-$match group then filter (TPC-H Q13 style)')
    ...
    @query('group', 1.0, 'Partsupp group by supplier cost < 100')
    ...
    @query('group', 1.0, 'Supplier group by nationkey')
    ...
    @query('limit', 1.0, 'Lineitem top-1 most expensive 1995+')
    ...
    @query('limit', 1.0, 'Lineitem top-5000 most expensive 1995+')
    ...
    @query('limit', 1.0, 'Orders earliest order price > 100K (limit 1)')
    ...
    @query('limit', 1.0, 'Orders price > 100K sorted by date limit 10000')
