...
from latency_estimation.mongo.flat_dataset import MongoFlatArrayDataset, MongoFlatDatasetItem, save_mongo_flat_dataset
from latency_estimation.mongo.flat_feature_extractor import (
...
def main(rawArgs: list[str] | None = None):
    parser = argparse.ArgumentParser(description='Create a MongoDB flat-feature dataset from measured query JSONL files.')
    add_args(parser)
...
def add_args(parser: argparse.ArgumentParser):
    parser.add_argument('dataset_id', nargs=1, help='Id of the output dataset. Pattern: mongo/{dataset_name}.')
    ...
    parser.add_argument('--val-dataset', type=str, help='Optional validation dataset id. Pattern: mongo/{dataset_name}.')
    ...
    parser.add_argument('--refresh-queryplanner', action='store_true', help='Re-fetch MongoDB queryPlanner plans from the measured database ids without executing queries.')
    parser.add_argument('--include-schema-identifiers', action='store_true', help='Include collection, index, and field-name features. Off by default for cross-schema evaluation.')
...
def run(config: Config, args: argparse.Namespace):
    ...
    if val_dataset_id is not None:
        ...
        _require_mongo(val_driver_type)
    ...
    if args.feature_extractor_dataset:
        ...
        feature_extractor.fit([
            (measurement.plan, measured.global_stats)
            for measured in all_measured
            for measurement in measured.items
        ])

    items = list[MongoFlatDatasetItem]()
    for measured in all_measured:
        for measurement in measured.items:
            items.append(create_flat_dataset_item(feature_extractor, measurement, measured.global_stats, args.skip_first))
...
def create_flat_dataset_item(
    feature_extractor: FlatFeatureExtractor,
    measurement: QueryMeasurement[TQuery],
    global_stats: dict,
    skip_first: int,
) -> MongoFlatDatasetItem:
    ...
    return MongoFlatDatasetItem(
        query_id=measurement.id,
        label=measurement.label,
        latency=latency,
        features=feature_extractor.transform_plan(measurement.plan, global_stats),
        plan=measurement.plan,
        global_stats=global_stats,
    )


def split_dataset_items(dataset_items: list[MongoFlatDatasetItem], val_ratio: float, seed: int) -> tuple[list[MongoFlatDatasetItem], list[MongoFlatDatasetItem]]:
    rng = random.Random(seed)
    groups = dict[str, list[MongoFlatDatasetItem]]()
    for item in dataset_items:
        _, template_name, _ = parse_query_instance_id(item.query_id)
        groups.setdefault(template_name, []).append(item)

    train_items = list[MongoFlatDatasetItem]()
    val_items = list[MongoFlatDatasetItem]()
...
def _save_flat_dataset_artifacts(pp: PathProvider, dataset_id: DatasetId, items: list[MongoFlatDatasetItem], feature_extractor: FlatFeatureExtractor):
    save_mongo_flat_dataset(pp.flat_dataset(dataset_id), MongoFlatArrayDataset(items))
...
def _load_all_measured(pp: PathProvider, measurement_suffixes: list[str]):
    ...
    for suffix in measurement_suffixes:
        path = pp.measured_by_suffix(DriverType.MONGO, suffix)
        try:
    ...
    if not all_measured:
        exit_with_error(f'No valid MongoDB measurement files found for suffixes {measurement_suffixes}.')

    return all_measured


def _refresh_queryplanner_plans(config: Config, all_measured):
...
def _require_mongo(driver_type: DriverType):
    if driver_type != DriverType.MONGO:
        raise ValueError('Flat-feature tree models are implemented only for MongoDB datasets in this script.')
