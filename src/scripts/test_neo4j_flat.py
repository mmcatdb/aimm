...
from latency_estimation.neo4j.flat_model import compute_metrics, load_flat_model, print_metrics, transform_dataset_features
from providers.path_provider import PathProvider


def main(rawArgs: list[str] | None = None):
    parser = argparse.ArgumentParser(description='Evaluate a Neo4j flat-feature tree latency model on a flat dataset.')
    add_args(parser)
...
def add_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--output', type=str, help='Optional JSON output path. Defaults to data/neo4j_flat_evaluation_results.json.')
...
def run(config: Config, args: argparse.Namespace):
    ...
    if driver_type != DriverType.NEO4J:
        raise ValueError('Neo4j flat-feature tree evaluation requires a model id with pattern neo4j/{model_name}.')

    dataset_id = create_dataset_id(driver_type, args.test_dataset[0])
    dataset = load_neo4j_flat_dataset(pp.flat_dataset(dataset_id))
    ...
    output = args.output or os.path.join(config.results_directory, 'neo4j_flat_evaluation_results.json')
